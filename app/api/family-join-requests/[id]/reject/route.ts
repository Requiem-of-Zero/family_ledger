...
import { rejectFamilyJoinRequest } from "@/src/server/services/families.service";

const FamilyJoinRequestIdSchema = z.coerce.number().int().positive();

// State transition route: invited user declines a pending family invitation.
export async function POST(
  ...
  try {
    ...
    const familyJoinRequest = await rejectFamilyJoinRequest(user.id, parsedId.data);
    ...
    console.error("POST /api/family-join-requests/[id]/reject error:", error);
    return NextResponse.json(
