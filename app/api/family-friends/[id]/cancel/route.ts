...
import { cancelFamilyFriendRequest } from "@/src/server/services/families.service";

const FamilyFriendIdSchema = z.coerce.number().int().positive();

// State transition route: requesting family owner cancels a pending request.
export async function POST(
  ...
  try {
    ...
    const result = await cancelFamilyFriendRequest(user.id, parsedId.data);
    ...
    console.error("POST /api/family-friends/[id]/cancel error:", error);
    return NextResponse.json(
