...
import { rejectFriendRequest } from "@/src/server/services/friends.service";

const FriendRequestIdSchema = z.coerce.number().int().positive();

// State transition route for the receiving user. Rejecting currently deletes
...
export async function POST(
  ...
  try {
    ...
    const result = await rejectFriendRequest(user.id, parsedId.data);
    ...
    console.error("POST /api/friends/[id]/reject error:", error);
    return NextResponse.json(
